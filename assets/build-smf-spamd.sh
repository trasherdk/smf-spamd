...
#rm -fr ${BASE}/${softname}*
echo "Remove old stuff: ${BASE}/${softname}-*"
for F in $(ls -d -1 "${BASE}/${softname}-*" 2> /dev/null); do 
	if [ -d ${F} -o -f ${F} ]; then
		echo -n "${WHITE}* Deleting ${F}:${RESTORE}"; 
		#rm -fr "${BASE}/$(basename${F})" \
		echo "${BASE}/$(basename${F})" \
		&& { echo "${LGREEN} OK ${RESTORE}"; } \
		|| { echo "${RED} FAIL ${RESTORE}"; exit 1; }
	fi
done

if [ ! -d ${BASE}/${softname} ]; then
	mkdir ${BASE}/${softname}
	cd ${BASE}/${softname}
	
	FILE=$(ls -1 ${BASE}/packages/${softname}-*.tar.gz | tail -n1)
	echo "${YELLOW}* Extract to ${softname}:${WHITE} ${FILE} ${RESTORE}"
	tar --strip-components=1 -xvf ${FILE}
	cd ${BASE}
...
cp -ra ${BASE}/$(basename $0) ${BASE}/assets contrib init COPYING ChangeLog readme \
...
find . -print0 | xargs --null file | grep "executable" | grep ELF | cut -f 1 -d : | xargs strip --strip-unneeded 2> /dev/null
find . -print0 | xargs --null file | grep "shared object" | grep ELF | cut -f 1 -d : | xargs strip --strip-unneeded 2> /dev/null
